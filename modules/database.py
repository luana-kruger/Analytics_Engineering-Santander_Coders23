...
def criar_schemas():
    try:
        ...
        with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
            ...
            sql_exists_landing = "SELECT schema_name FROM information_schema.schemata WHERE schema_name = 'landing'"
            exists_landing = conn.execute(text(sql_exists_landing)).fetchone()
            if not exists_landing:
                sql_create_landing = "CREATE SCHEMA landing"
                conn.execute(text(sql_create_landing))
                conn.commit()
